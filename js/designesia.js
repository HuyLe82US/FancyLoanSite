...
// --------------------------------------------------
	...
	jQuery('.timer').each(function() {
        ...
        if (imagePos < topOfWindow + jQuery(window).height() && !jQuery(this).hasClass('animated-count')) {
            jQuery(this).addClass('animated-count'); // Avoid re-running the animation
            jQuery(this).countTo({
                from: 0,                // Start at 0
                to: jQuery(this).data('to'),  // End at the value set in the `data-to` attribute
                speed: 1000,            // Duration of animation in milliseconds
                refreshInterval: 50     // How often the number is updated
            });
        }
    });
	});
